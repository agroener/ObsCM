...
mvir_tbd = [i for i in range(len(mvir)) if mvir[i] in ['TBD','nan'] and methods[i] in ['WL','SL','WL+SL']]
cvir_tbd = [i for i in range(len(cvir)) if cvir[i] in ['TBD','nan'] and methods[i] in ['WL','SL','WL+SL']]
m200_tbd = [i for i in range(len(m200)) if m200[i] in ['TBD','nan'] and methods[i] in ['WL','SL','WL+SL']]
c200_tbd = [i for i in range(len(c200)) if c200[i] in ['TBD','nan'] and methods[i] in ['WL','SL','WL+SL']]
...
mvir_lens = [mvir[i]*1e14 for i in range(len(mvir)) if i not in tbd_vir and methods[i] in ['WL','SL','WL+SL']]
cvir_lens = [cvir[i] for i in range(len(cvir)) if i not in tbd_vir and methods[i] in ['WL','SL','WL+SL']]
m200_lens = [m200[i]*1e14 for i in range(len(m200)) if i not in tbd_200 and methods[i] in ['WL','SL','WL+SL']]
c200_lens = [c200[i] for i in range(len(c200)) if i not in tbd_200 and methods[i] in ['WL','SL','WL+SL']]

z_vir_lens = [redshift[i] for i in range(len(redshift)) if i not in tbd_vir and methods[i] in ['WL','SL','WL+SL']]
z_200_lens = [redshift[i] for i in range(len(redshift)) if i not in tbd_200 and methods[i] in ['WL','SL','WL+SL']]
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, a=3.0, 
                                args=(np.array(mvir_lens), np.array(cvir_lens), np.array(z_vir_lens)),
                                threads=2)
